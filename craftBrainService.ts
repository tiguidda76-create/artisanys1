/* ═══════════════════════════════════════════════════════════════
   MARRAKECH CRAFT BRAIN SERVICE (TypeScript Engine)
   Orchestrateur IA 360° & Intelligence Artisanale Déterministe
   ═══════════════════════════════════════════════════════════════ */

export type CraftCategory = 'rugs' | 'ceramics' | 'brass' | 'leather' | 'wicker' | 'wood' | 'mixed';

export type MedinaQuarter = 
  | 'Mouassine' 
  | 'Semmarine' 
  | 'Bab Doukkala' 
  | 'Sidi Ghanem' 
...
export interface ArtisanWorkshop {
  id: string;
  name: string;
  masterMaalem: string;
  category: CraftCategory;
  categoryLabel: string;
  quarter: MedinaQuarter;
  address: string;
  teamSize: number;
  specialties: string[];
...
export interface InventoryItem {
  ...
  unitPriceUsd: number;
  unitPriceMad: number;
  status: 'In Stock' | 'Low Stock' | 'Critical Reorder' | 'Made on Demand';
  quarter: MedinaQuarter;
}
...
export interface ExportOrder {
  ...
  shippingMethod: 'DHL Express Air' | 'Air Cargo Priority' | 'Ocean FCL (Casablanca)';
  status: 'Production' | 'Quality Check' | 'Customs Prep' | 'Dispatched' | 'Delivered';
  estimatedDeliveryDays: number;
  trackingNumber: string;
  maalemInCharge: string;
}
...
export interface FinancialKPIs {
  ...
  pipelineMad: number;
  pipelineEur: number;
  activeOrdersUsd: number;
  averageMarginPercent: number;
  totalWorkshops: number;
  criticalWorkshopsCount: number;
  lowStockItemsCount: number;
}

export interface ActionCardTrigger {
  type: 'workshop_profile' | 'whatsapp_direct' | 'check_stock' | 'pro_forma_quote' | 'track_order' | 'custom_search';
  label: string;
  icon: string;
...
export interface BrainResponse {
  ...
  voiceText: string;
  highlights?: string[];
  actionCards?: ActionCardTrigger[];
  customHtml?: string;
}

// ── Master Knowledge Base ──────────────────────────────────────

export const WORKSHOPS_DATABASE: ArtisanWorkshop[] = [
  {
...
export const EXPORT_ORDERS_DATABASE: ExportOrder[] = [
...
export class CraftBrainService {
  private workshops: ArtisanWorkshop[] = [...WORKSHOPS_DATABASE];
  private inventory: InventoryItem[] = [...INVENTORY_DATABASE];
  private orders: ExportOrder[] = [...EXPORT_ORDERS_DATABASE];
  private exchangeRates = { USD: 1.00, EUR: 0.92, MAD: 9.85, GBP: 0.79, AUD: 1.53 };
  ...
  public getFinancialKPIs(): FinancialKPIs {
    const pipelineUsd = this.orders.reduce((sum, o) => sum + o.totalAmountUsd, 0) + 48200; // includes CRM leads estimate
  ...
  public getWorkshops(): ArtisanWorkshop[] {
  ...
  public getOrders(): ExportOrder[] {
    return this.orders;
  }

  public findWorkshopById(id: string): ArtisanWorkshop | undefined {
    return this.workshops.find(w => w.id === id);
  }

  public findWorkshopsByQuarter(quarter: string): ArtisanWorkshop[] {
  ...
  public findWorkshopsByCategory(cat: string): ArtisanWorkshop[] {
  ...
  /**
   * Moteur NLP Déterministe & Traitement Contextuel 360°
   */
  public async processQuery(query: string): Promise<BrainResponse> {
    const q = query.toLowerCase().trim();

    // 1. Intent: Bilan Global / "Check Everything"
    ...
    // 4. Intent: Commandes Export / Fret / Expéditions
    ...
    // 5. Intent: Chiffre d'affaires & Finances
    ...
    // 6. Intent: Recherche par Quartier de la Médina
    const quarters: MedinaQuarter[] = ['Mouassine', 'Semmarine', 'Bab Doukkala', 'Sidi Ghanem', 'Kasbah', 'Rahba Kedima', 'Mellah'];
    ...
    // 7. Intent: Recherche par Métier Artisanal
    const craftMap: Record<string, CraftCategory> = {
      'tapis': 'rugs',
      'kilim': 'rugs',
      'berbere': 'rugs',
      'beni ourain': 'rugs',
    ...
    // 8. Intent: Recherche par Nom de Maâlem
    ...
    // 9. Intent: Devis Pro Forma / Facturation
    ...
    // 10. Fallback Intelligent & Recommandations
    return this.handleSmartFallback(query);
  }

  // ── Handler Implementations ────────────────────────────────────

  private handleGlobalBilan(query: string): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = [
      {
        type: 'workshop_profile',
        label: 'Voir Ateliers Critiques',
        icon: '⚠️',
        payload: { filter: 'critical' }
      },
      {
        type: 'check_stock',
        label: 'Gérer les Réappros',
        icon: '📦',
        payload: { action: 'open_catalog' }
      },
      {
        type: 'track_order',
        label: 'Suivi des Expéditions',
        icon: '🚢',
        payload: { action: 'view_orders' }
      },
      {
        type: 'pro_forma_quote',
        label: 'Émettre un Devis Pro Forma',
        icon: '💼',
        payload: { action: 'open_invoice' }
      }
    ];
  ...
  private handleStockBilan(query: string): BrainResponse {
    const lowStock = this.inventory.filter(i => i.status === 'Critical Reorder' || i.status === 'Low Stock');
    ...
    const actionCards: ActionCardTrigger[] = lowStock.map(item => ({
  ...
  private handleCriticalArtisans(query: string): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = critical.map(w => ({
      type: 'whatsapp_direct',
  ...
  private handleExportOrders(query: string): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = [
      {
        type: 'track_order',
        label: 'Calculer Fret Express DHL',
        icon: '✈️',
        payload: { action: 'open_freight_calc' }
      },
      {
        type: 'pro_forma_quote',
        label: 'Générer Facture Pro Forma',
        icon: '📄',
        payload: { action: 'open_invoice' }
      }
    ];
  ...
  private handleFinancialKpis(query: string): BrainResponse {
    const kpis = this.getFinancialKPIs();
    ...
    const actionCards: ActionCardTrigger[] = [
      {
        type: 'pro_forma_quote',
        label: 'Créer Devis Export Pro Forma',
        icon: '💼',
        payload: { action: 'create_quote' }
      },
      {
        type: 'check_stock',
        label: 'Simulateur Remises Volume',
        icon: '📐',
        payload: { action: 'open_simulator' }
      }
    ];
  ...
  private handleQuarterSearch(query: string, quarter: MedinaQuarter): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = matching.map(w => ({
      type: 'workshop_profile',
  ...
  private handleCraftSearch(query: string, category: CraftCategory, keyword: string): BrainResponse {
  ...
  private handleSingleWorkshop(query: string, w: ArtisanWorkshop): BrainResponse {
  ...
  private handleProFormaIntent(query: string): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = [
      {
        type: 'pro_forma_quote',
        label: 'Ouvrir Onglet Facturation & Devis',
        icon: '📑',
        payload: { action: 'open_legal' }
      },
      {
        type: 'check_stock',
        label: 'Simulateur de Prix & Remises',
        icon: '📐',
        payload: { action: 'open_simulator' }
      }
    ];
  ...
  private handleSmartFallback(query: string): BrainResponse {
    ...
    const actionCards: ActionCardTrigger[] = [
      {
        type: 'workshop_profile',
        label: 'Bilan Global 360°',
        icon: '📊',
        payload: { action: 'check_everything' }
      },
      {
        type: 'check_stock',
        label: 'Audit des Stocks',
        icon: '📦',
        payload: { action: 'stock_audit' }
      },
      {
        type: 'custom_search',
        label: 'Ateliers Sidi Ghanem',
        icon: '📍',
        payload: { query: 'Sidi Ghanem' }
      },
      {
        type: 'custom_search',
        label: 'Tapis & Beni Ourain',
        icon: '🧶',
        payload: { query: 'tapis' }
      }
    ];
...
export const craftBrainService = new CraftBrainService();

// Export to Browser Window for Direct Client-Side Compatibility
if (typeof window !== 'undefined') {
  (window as any).CraftBrainService = craftBrainService;
}
